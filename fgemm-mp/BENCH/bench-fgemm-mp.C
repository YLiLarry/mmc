...
void run_bench(size_t m, size_t n, size_t k, size_t b, size_t iters, size_t seed){
	...
	for (;( loop<iters || std::chrono::duration<double>(end-start) < std::chrono::seconds(1)) ;loop++){
...
#ifdef BENCH_MMX
		{
			...
			xnat sz1= matrix_crt_multiply_helper<mT>
				::size (tab(AAA), Mat::index (1, 0, m, k), Mat::index (0, 1, m, k),m, k);
			xnat sz2= matrix_crt_multiply_helper<mT>
				::size (tab(BBB), Mat::index (1, 0, k, n), Mat::index (0, 1, k, n),k, n);
			xnat sz= matrix_crt_multiply_helper<mT>
				::size (tab(AAA), Mat::index (1, 0, m, k), Mat::index (0, 1, m, k),

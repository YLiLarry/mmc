...
void test5()
{
    const size_t phase1_moduli_count = 5;
    ...
    a->val(0) = LInteger::random_exact(100);
    a->val(1) = LInteger::random_exact(100);
    a->val(2) = LInteger::random_exact(100);
    a->val(3) = LInteger::random_exact(100);
    LIntegerPtrVector *b = LIntegerPtrVector::new_zeros(4);
    b->val(0) = LInteger::random_exact(100);
    b->val(1) = LInteger::random_exact(100);
    b->val(2) = LInteger::random_exact(100);
    b->val(3) = LInteger::random_exact(100);
    cerr << "input a: " << *a << endl;
    cerr << "input b: " << *b << endl;
    ...
    _time = clock();
    vector<Givaro::Integer> expect = SIM_RNS::fflas_mult_integer(*_a, *_b, 2, 2, 2);
    _time = clock() - _time;
    cerr << "It took me " << _time << " clicks (" << static_cast<float>(_time) / CLOCKS_PER_SEC << " seconds)" << endl;
    if (!p->equals(expect))
    {

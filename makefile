...
C_FLAGS := $(DEBUG_FLAGS) -ferror-limit=1 -g -c -Wall -lgmp
C_TARGETS := ./cnma/*.c 
C_OBJECTS := *.o 
CPP_FLAGS := $(DEBUG_FLAGS) -ferror-limit=1 -g -Wall --std=c++14 -I"$(LINBOX_INCLUDE)" -I"$(GIVARO_INCLUDE)" -I"$(FFLAS_INCLUDE)" -L"$(LINBOX_LIB)" -L"$(GIVARO_LIB)" -L"$(BLAS_LIB)" -L"$(FFLAS_LIB)" -lgivaro -lopenblas -llinbox -lgmp 
